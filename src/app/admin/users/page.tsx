...
function EditModal({ user, locations, roleOptions, onClose, onSaved }: {
  user: UserProfile; locations: Location[]; roleOptions: Option[]; onClose: () => void; onSaved: () => void;
}) {
  const [form, setForm] = useState({
    name: user.name, email: user.email, role: user.role, group: user.group,
    locationId: user.location ? String(user.location.id) : '',
  });
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl">
        ...
        <div className="flex flex-col gap-3 mb-4">
          ...
          <input className={inputClass} placeholder="Group" value={form.group} onChange={(e) => setForm((p) => ({ ...p, group: e.target.value }))} />
          <select className={inputClass + ' bg-white'} value={form.locationId} onChange={(e) => setForm((p) => ({ ...p, locationId: e.target.value }))}>
            <option value="">No location assigned</option>
            {locations.map((l) => <option key={l.id} value={l.id}>{l.building} — {l.floor}, {l.zone} ({l.city})</option>)}
          </select>
        </div>
...
export default function AdminUsersPage() {
  ...
  async function addRole() {
    ...
    if (!val) return;
    await fetch('/api/dropdown-options', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ category: 'role', value: val }) });
    setNewRole('');
    load();
  }
  ...
  async function addProfile() {
    ...
    const r = await fetch('/api/user-profiles', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, locationId: form.locationId || null }) });
  ...
  async function addVehicle() {
    ...
    await fetch('/api/vehicles', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userProfileId: Number(vForm.userProfileId), type: vForm.type, plateNumber: vForm.plateNumber.trim() }) });
  ...
  return (
    <div className="p-5 sm:p-7 md:p-8 max-w-4xl">
      {editUser && <EditModal user={editUser} locations={locations} roleOptions={roleOptions} onClose={() => setEditUser(null)} onSaved={load} />}
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm ring-1 ring-gray-100 mb-4">
        <button onClick={() => setShowManageRoles((v) => !v)} className="flex items-center justify-between w-full text-left">
        ...
        {showManageRoles && (
          <div className="mt-4">
            ...
            <div className="flex gap-2">
              <input className="flex-1 px-3 py-1.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1e3a5f]"
                placeholder="New role..." value={newRole} onChange={(e) => setNewRole(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && addRole()} />
      ...
      {profiles.length === 0 && <div className="bg-white rounded-2xl p-10 text-center shadow-sm ring-1 ring-gray-100 text-gray-400">No user profiles yet.</div>}
          ...
          <div key={p.id} className="bg-white rounded-2xl shadow-sm ring-1 ring-gray-100 overflow-hidden">
            <div className="px-5 py-4 flex items-center justify-between gap-4">
              ...
              <div className="flex gap-3 items-center flex-shrink-0">
                <button onClick={() => setExpanded(expanded === p.id ? null : p.id)} className="text-sm text-[#1e3a5f] hover:underline font-medium whitespace-nowrap">
                  {expanded === p.id ? 'Hide' : `Vehicles (${p.vehicles.length})`}
                </button>
                <button onClick={() => setEditUser(p)} className="text-sm text-gray-500 hover:text-gray-800 font-medium">Edit</button>
            ...
            {expanded === p.id && (
              <div className="border-t border-gray-100 px-5 py-4 bg-gray-50">
                ...
                {p.vehicles.length === 0 && <p className="text-xs text-gray-400">No vehicles yet.</p>}
