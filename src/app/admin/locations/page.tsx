...
export default function AdminLocationsPage() {
  ...
  async function addLocation() {
    ...
    if (!country || !city || !building || !floor || !zone) {
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm ring-1 ring-gray-100 mb-6">
        ...
        {opts('country').length === 0 && (
          <p className="text-xs text-amber-600 mb-3">
            No options available. <a href="/admin/manage-lists" className="underline font-medium">Add options in Manage Lists</a> first.
          </p>
        )}
        <button onClick={addLocation} className="px-5 py-2 bg-[#1a2535] text-white rounded-lg text-sm font-medium hover:bg-[#243148] transition-colors">
          Add Location
        </button>
      </div>
