  ...
  return (
    <div className="px-4 py-6 sm:px-8 sm:py-8 md:px-12 md:py-12 max-w-3xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h2 className="text-xl sm:text-2xl font-semibold text-gray-900 tracking-tight">Shift Schedules</h2>
      </div>
      ...
      {shifts.length === 0 && (
        <div className="bg-white rounded-2xl p-10 text-center shadow-sm ring-1 ring-gray-100 text-gray-400">No shifts yet.</div>
      )}
      <div className="flex flex-col gap-3">
        {shifts.map((s) => (
          <div key={s.id} className="bg-white rounded-xl px-5 py-4 shadow-sm ring-1 ring-gray-100 flex items-center justify-between gap-4">
            ...
            <button onClick={() => remove(s.id)} className="text-red-500 text-sm hover:text-red-700 font-medium flex-shrink-0">Remove</button>
          </div>
        ))}
      </div>
    </div>
